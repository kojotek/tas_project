...
namespace MyWCFServices
{
    public class BiedaService : IBiedaService, helpfullThings
    {
        ...
        public List<string> getOffers(int id_aukcji)
        {
            connect();
            List<string> lista = new List<string>();
            ...
            try
            {
                SqlCommand cmd = new SqlCommand("select * from Oferta where id_aukcji = " + id_aukcji.ToString() + " order by kwota desc", conn);
                SqlDataReader dr = cmd.ExecuteReader();
                while (dr.Read())
                {
                    lista.Add(dr["login"].ToString());
                    lista.Add(dr["kwota"].ToString());
                    lista.Add(dr["data_zlozenia"].ToString());
                }
            }
            finally
            {
